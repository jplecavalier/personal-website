...
On a maintenant tout ce dont on a de besoin pour créer une carte illustrant le revenu total médian des ménages en 2015 par aire de diffusion agrégée.
...
Je vous invite à consulter la documentation du *package* pour comprendre l'effet des différentes fonctions et leurs arguments respectifs.
...
argument, l'échelle qu'on a défini est linéaire entre le minimum (`r formatC(min(ada_qc[, med_income]), format="d", big.mark=" ")` \$) et le maximum
...
de cet intervalle seront tronquées à l'aide de la fonction `squish()`.
...
Pour donner un effet de surbrillance intéressant, on va augmenter l'opacité du remplissage et foncir et épaissir les contours du polygone. Pous les étiquettes,
...
afficher au passage de la souris. J'invite les lecteurs un peu plus expérimentés à l'essayer!
...
Ça faisait déjà plusieurs fois que des collègues m'avaient demandé d'écrire un article avec quelques-uns de mes trucs personnels pour créer de belles cartes en
<i class="fab fa-r-project"></i>. J'espère que celui-ci vous a plu et, surtout, qu'il vous sera utile lorsque vous créerez vos prochaines cartes!
