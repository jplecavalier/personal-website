...
On commence donc par charger différentes informations sur chaque équipe (dont l'emplacement de leur amphithéâtre et leur division respective) à l'aide de la
fonction `tidy_teams_meta()`. On calculera ensuite une matrice des distances qu'on transformera en table pour en faciliter l'interrogation.
...
saison. Cependant, la distribution à l'intérieur de cette division est assez stable, ce qui n'est
...
Une chose est certaine, si le voyagement s'avère avoir un effet important sur la performance des
joueurs en séries, la division est partira avec un avantage considérable sur ses adversaires au
printemps prochain.
...
On applique donc une à la suite de l'autre les fonctions créées dans la section précédente. Ensuite,
on calcule la moyenne de kilomètres annuels parcourus et le nombre de kilomètres par jour moyen de
chaque équipe sur ces 7 saisons.
...
teams_travel_summary_20132020 <- teams_travel_summary_20132020[, .(
  travels_avg_km = mean(travels_sum_km),
  travels_avg_km_days = sum(travels_sum_km) / sum(season_days)
), .(team, division)]
...
On peut finalement recréer le même graphique qu'à la section précédente, mais reflétant la moyenne
des saisons 2013-14 à 2019-20 au lieu de la saison 2021.

```{r plot_summary_201320, fig.height=8}
ggplot(
  data = teams_travel_summary_20132020,
  mapping = aes(
    x = travels_avg_km,
    y = reorder(as.factor(team), travels_avg_km),
    fill = division
  )
) +
  geom_col() +
  ...
  scale_fill_brewer(palette = "Set1") +
  labs(
    title = "Distance annuelle moyenne parcourue par équipe",
    subtitle = "Saisons 2013-14 à 2019-20 de la LNH",
    x = "Distance annuelle moyenne (km)"
  ) +
  theme(
    panel.grid.major.y = element_blank(),
    legend.title = element_blank(),
