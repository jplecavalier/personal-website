...
d'une fonction afin d'être réutilisées un peu plus loin dans l'article.
...
l'endroit du dernier match. On applique cette fonction à nos données dans le bloc ci-dessous.
...
```{r mtl_head}
teams_travel_2021[team == "MTL"][1:4]
```
...
create_teams_travel_summary <- function(teams_travel, nhl_schedule) {
  
  # Créer un tableau sommaire
  teams_travel_summary <- teams_travel[, .(
    travels_nb = .N,
  ...
  )][, .(season_days = as.integer(max(game_date) - min(game_date) + 1L)), season_years]
  teams_travel_summary[nhl_season_days, season_days := season_days, on = c(season = "season_years")]
  teams_travel_summary[, travels_km_days := travels_sum_km / season_days]
...
teams_travel_summary_2021 <- create_teams_travel_summary(teams_travel_2021, nhl_schedule_2021)
...
ggplot(
  ...
  mapping = aes(
    x = travels_sum_km,
    y = reorder(as.factor(team), travels_sum_km),
    fill = division
  )
) +
  geom_col() +
...
parcourir pendant la saison, les Blues en parcoureront près de 35 000 (plus du double!) pendant la
même période. Reste à voir si ça donnera un assez gros coup de main aux Ducks pour qu'ils soient en
mesure de compenser pour leur manque de talent offensif.

Une chose est certaine, si la fatigue accumulée due au voyagement pendant la saison s'avère être un
bon indicateur de la performance des joueurs en séries, la division est partira avec un avantage
considérable sur ses adversaires au printemps prochain.
...
teams_schedule_20132020 <- create_team_view(nhl_schedule_20132020)

# Transformation des données
add_last_game(teams_schedule_20132020)

# Créer une vue des déplacements
teams_travel_20132020 <- create_travel_view(teams_schedule_20132020)

# Créer le sommaire des déplacements par équipe par saison
teams_travel_summary_20132020 <- create_teams_travel_summary(teams_travel_20132020, nhl_schedule_20132020)

# Créer le sommaire des déplacements par équipe
teams_travel_summary_20132020 <- teams_travel_summary_20132020[, .(
  season = "2013-20",
  travels_sum_km = mean(travels_sum_km),
  travels_km_days = sum(travels_sum_km) / sum(season_days)
), .(team, division)]
...
teams_travel_summary <- copy(teams_travel_summary_2021[, .(team, division, km_day_2021 = travels_km_days)])
teams_travel_summary[teams_travel_summary_20132020, km_day_201320 := travels_km_days, on = .(team)]
teams_travel_summary[, travel_cut := (km_day_2021 / km_day_201320) - 1]

ggplot(
  data = teams_travel_summary,
  mapping = aes(
    y = reorder(as.factor(team), travel_cut),
    fill = division
  )
) +
  geom_col(aes(x = km_day_2021), alpha = 1) +
  geom_col(aes(x = km_day_201320), alpha = 0.5) +
  geom_text(
    mapping = aes(
      x = km_day_2021,
      label = percent(travel_cut, 1)
    ),
    nudge_x = 2,
    hjust = 0
  ) +
...
Sans surprise, on voit que le fait de disputer plusieurs matchs consécutifs contre la même équipe
réduit le déplacement annuel pour l'ensemble des équipes. Les grands gagnants de ce remaniement sont
les équipes de la division est et ceux en retirant les bénéfices les moins importants (quoi que tout
de même significatifs) sont principalement les équipes canadiennes. Mon coeur de partisan des
Canadiens est toujours satisfait de voir les Leafs en dernière position d'un classement quel qu'il
soit `r emoji("beaming_face_with_smiling_eyes")`.
