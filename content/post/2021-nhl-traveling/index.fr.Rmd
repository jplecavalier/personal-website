...
On crée ensuite un sommaire par équipe indiquant le voyagement total en km pour la saison 2021 auquel on vient ajouter la division à laquelle appartient chaque
équipe pour la saison en cours.
...
teams_travel_summary_2021 <- teams_travel_2021[, .(
  travels_nb = .N,
  travels_sum_km = sum(distance)
), team]

# Fusionner les divisions
teams_travel_summary_2021[teams_meta, division:=division_active_name, on=c(team="team_abbreviation")]

# Afficher un aperçu
teams_travel_summary_2021[]
```

On visualise l'information dans un graphique.

```{r plot_summary_2021, fig.height=7}
library(ggplot2)
library(scales)
...
ggplot(
  ...
  labs(
    x = "Distance totale à parcourir (km)",
    caption = "Saison 2021 de la LNH"
  ) +
  theme_minimal() +
  theme(
